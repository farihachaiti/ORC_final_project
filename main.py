...
if __name__=='__main__':
    ...
    for i in range(10):
        ...
        x_init[nq:] = (np.random.rand(nq) - 0.5) * 0.5
        ...
        J_X_init.append(torch.tensor([J_val], dtype=torch.float32))
    ...
    print("input tensor shape", x_init)
    ...
    with torch.no_grad():
        ...
        for i in range(5, 10):
            ...
            J_terminal = nn_func(X_init_val_casadi[i])  # Convert to Python scalar
            print(f"Final predicted terminal cost: {J_terminal}")
    '''M = 50
